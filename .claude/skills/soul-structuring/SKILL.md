...
### 7. 패키징 — 업로드용 zip (마지막 단계)
사용자는 이 zip을 **앱에서 직접 업로드**해 트랙을 추가한다(트랙 추가 → ZIP 업로드, [화면구조도](../../../docs/frontend-screens.md)·[파이프라인 §3](../../../docs/data-pipeline.md)). 그래서 스킬의 산출물은 디렉토리가 아니라 **zip 하나**다.
```bash
python3 .claude/skills/soul-structuring/scripts/pack_soul.py .soul/{trackSlug}
# → .soul/{trackSlug}.zip  (manifest.json + decks/ 를 zip 루트에 담음)
```
- 패커는 **먼저 검증을 다시 돌려** 통과한 번들만 압축한다(잘못된 zip 차단). zip은 결정적(재패킹 시 동일).
- 결과 zip의 경로를 사용자에게 알려준다: **"앱 → 트랙 추가 → ZIP 업로드"에 이 파일을 올리면 된다.**
- `examDate`는 zip에 없다 — 업로드 후 앱의 **시험일 설정** 화면에서 입력한다.
